/*
 * Joe Subbiani, 2377990s, SP Exercise 2
...
struct theTableStruct{
private:
    std::unordered_map<std::string, std::list<std::string>> theTable;
    std::mutex mutex; // mutex to protect the critical section

public: auto find(const std::string& s){
...
public: void insert(std::pair<std::string, std::list<std::string>> p) {
        std::unique_lock<std::mutex> lock(mutex); // lock mutex: enter critical section
        theTable.insert(p);
    }
public:
    std::list<std::string>& operator[] (const std::string& s) {
...
struct workQStruct{
private: std::list<std::string> workQ;
    std::mutex mutex; // mutex to protect the critical section

public: void push_back(const std::string& value) {
        std::unique_lock<std::mutex> lock(mutex); // lock mutex: enter critical section
...
struct theTableStruct theTable;
struct workQStruct workQ;
...
int main(int argc, char *argv[]) {
  ...
  std::mutex mutex; //lock the part of the loop getting the filename

  // 4. for each file on the workQ
  std::vector<std::thread> threads;
  for(i=0; i<CRAWLER_THREADS; i++)
    {
        threads.push_back(std::thread([mutex_ptr = &mutex](){
            while ( workQ.size() > 0 ) {
